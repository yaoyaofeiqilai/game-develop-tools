...
- If global strong refinement still leaves background, use `refine-region`. It crops the original source around the polygon and reruns the configured remover, but writes back only candidates also supported by full-image canvas colour, flatness, and semantic background evidence. Start with `standard`; higher strengths broaden pale-background evidence without treating the whole polygon as an eraser.
...
- Commands that create or export results through this client persist the `ai` tag on the job. Those jobs appear in the GUI's “AI处理记录” tab for focused human review and can be reopened for mask or assignment edits. Do not remove that tag merely because a later manual edit occurs.
