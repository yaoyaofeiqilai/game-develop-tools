...
- `POST /api/jobs/{job_id}/refine` with `{"mode":"balanced","tolerance":18,"matte_width":3}` rebuilds the mask from the preserved initial cutout without rerunning segmentation.
...
- `GET /api/jobs` lists processing history, including `tags` and the most recent `last_export`. Filter jobs whose tags contain `ai` for the AI processing record.
