...
- 服务默认只监听 `127.0.0.1`。
- 核心抠图和分割流程在本机运行，不需要上传图片。
- GUI 中填写的代理、视觉 API 地址和密钥保存在被忽略的 `workspace/` 中。
- `input/`、`output/`、模型、虚拟环境、任务历史、日志和本地密钥默认不会进入 Git。
- 在推送前仍建议运行 `git status --short`，确认没有手工强制添加的个人素材或凭据。

## 发布说明

仓库目前没有附带开源许可证。若准备把项目作为开源软件发布，请在推送前选择合适的许可证并添加 `LICENSE`；同时确认 `test/` 中回归素材拥有可公开分发的授权。
